...
# Removes symbols from alignment files
...
def generate_scores_for_sample(phone_times, frame_level_scores):
    scores = []
    #Iterate over phone transcription and calculate score for each phone
    for phone_name, start_time, end_time in phone_times:
        ...
        if start_time != end_time:
            # Calls this function from finetuning utils
...
def generate_scores_for_testset(model, testloader):
    ...
    for i, batch in enumerate(testloader, 0):       
        ...
        outputs     = (-1) * model(features, loss_per_phone, evaluation, batch_cum_matrix)
